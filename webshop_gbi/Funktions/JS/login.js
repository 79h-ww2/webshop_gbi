 ...
 */
function vorcheckEingabe_login(){
	...
	for ( var i = 0; i < felder.length; i++){
		if  ( document.forms['form_login'][felder[i]].value == ''){
			nicht_ausgefuellte_felder.push(meldungswort[i]);
			
			//Textbox rot umranden
			document.forms['form_login'][felder[i]].style.borderColor = 'red';
			document.forms['form_login'][felder[i]].style.borderStyle = 'solid';
		}
		else{
			//alte Textboxumrandung löschen, falls der Fehler behoben wurde
			document.forms['form_login'][felder[i]].style.borderColor = '';
			document.forms['form_login'][felder[i]].style.borderStyle = '';
		}
	}
	...
	//wenn alle Felder ausgefüllt sind, wird überprüft, ob die Eingaben gültig sind
		...
		if ( checkEmail(document.forms['form_login']['anmedlung_email'].value) == false ){
			fehlermeldung += fehlermeldung == '' ? '' : ' ';
			fehlermeldung += "Die Email-Adresse ist ung&uuml;ltig.";
			
			//Textbox rot umranden
			document.forms['form_login']['anmedlung_email'].style.borderColor = 'red';
			document.forms['form_login']['anmedlung_email'].style.borderStyle = 'solid';
		}
	}
	
	//zeigt die Fehlermeldung auf der Webseite an
	if (fehlermeldung != ''){
		document.getElementById('fehlermeldung_login').innerHTML = fehlermeldung;
	}
